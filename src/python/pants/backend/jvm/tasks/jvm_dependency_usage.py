...
class JvmDependencyUsage(JvmDependencyAnalyzer):
  ...
  def register_options(cls, register):
    ...
    register('--use-cached', action='store_true', help='Use only cached value, fail otherwise.')
  ...
  def execute(self):
    ...
    graph = DependencyUsageGraph(self.create_dep_usage_nodes(targets, get_buildroot()),
  ...
  def create_dep_usage_nodes(self, targets, buildroot):
    ...
    with self.invalidated(targets) as invalidation_check:
      ...
      if not self.get_options().use_cached:
        ...
        def calculating_node_creator(target):
          node = self.create_dep_usage_node(target, buildroot,
